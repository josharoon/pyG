...
from loss_functions import distance_field_loss

from .losses import *
from .utils import *
from .chamfer import nn_distance_function as chamfer_dist
from dfUtils.cubicCurvesUtil import distance_to_curves, convert_to_cubic_control_points, create_grid_points
...
class Trainer:
    ...
    def get_loss(self, img_inp, labels,ncoords,df):
        if type(img_inp) != list:
            ...
            #plot the img_inp tensor to window
            plt.imshow(img_inp[0].cpu().detach().numpy()[0])
            #plot the labels tensor to window
            #points=convert_to_cubic_control_points(labels[0].reshape(5,6).unsqueeze(0))
            #plotCubicSpline(points,"spline before training")




            inputs = labels
            outputs = self.network(img_inp,inputs)
    ...
    def _get_loss_pt(self, inputs, outputs, labels, nCoords,df):
        ...
        def get_distancefield_loss(input1, input2, input2_df):
            ...
            #temp hack to reverse Normalization
            input1 = input1 * 224
            input2 = input2 * 224
        ...
        def compute_occupancy_fields(distance_fields, eps=(2 / 224) ** 2):
            ...
            occupancy_fields = 1 - th.clamp(distance_fields/(224*32) / eps, 0, 1)
        ...
        surfLossVal += surfaceloss
        alignLossVal += alignmentloss
        ...
        # if global step is even print the df loss, surface, loss , alignment loss and dice loss

        self.writer.add_scalar('dfLoss', dfLossVal, self.global_step)
        self.writer.add_scalar('surfaceLoss', surfaceloss, self.global_step)
        self.writer.add_scalar('alignmentLoss', alignmentloss, self.global_step)
        # write tangent loss and point losses to tensor board
        self.writer.add_scalar('pointLoss', point_loss, self.global_step)
        self.writer.add_scalar('tangentLoss', tangent_loss, self.global_step)
        self.writer.add_scalar('diceLoss', diceLossVal, self.global_step)
